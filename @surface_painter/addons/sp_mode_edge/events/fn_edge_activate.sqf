...
["edge", "onEachFrame", {
	...
	if (SP_var_primaryMouseButton && {count SP_var_edge_line > 0}) then {
		drawLine3D [SP_var_edge_line select ((count SP_var_edge_line) - 1), SP_var_mouseWorldPosition, [1, 1, 1, 1]];
	};

	// drawn all the segments
	if (count SP_var_edge_line > 1) then {
		for [{_i = (count SP_var_edge_line) - 1}, {_i > 0}, {_i = _i - 1}] do {
			drawLine3D [SP_var_edge_line select _i, SP_var_edge_line select (_i - 1), [1, 1, 1, 1]];
		};
	};
